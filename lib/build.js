...
import tweetEmbed from 'metalsmith-tweet-embed';
...
metalsmith(path.resolve(__dirname, '../'))
  ...
  .use(files => {
    // include original file path in metadata
    Object.keys(files).forEach(file => {
      files[file].originalFile = file;
    });
  })
  .use(collections({
    posts: {
  ...
  .use(tweetEmbed({ pattern: '**/*.md', options: { align: 'center', dnt: true, omit_script: true } }))
  .use(markdown({
    highlight: hljs
  }))
  .use(anchorLinks())
